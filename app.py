...
@app.route('/', defaults={'car_make': None})
@app.route('//<car_make>', methods=['GET', 'POST'])
def index(car_make):
    cars = cars_make.find()
    #nos_of_cars = cars.count()
    #print(nos_of_cars)
    if car_make:
        car_models = cars_model.find({'car_make': car_make})
        return render_template('index.html', cars=cars, car_models=car_models)
    return render_template('index.html', cars=cars)


@app.route('/user_Car_view')
def user_car_view():
    car_models = basic_car_info.find({'car_model': request.form['car_model']})
    #for car in car_models:
       # print(car)
    return 'tehere are cars'
